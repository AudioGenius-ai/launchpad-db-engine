...
function getHookNames(tableName) {
  const pluralPascal = pascalCase2(tableName);
  const singularPascal = pascalCase2(singularize(tableName));
  return {
...
function generateTableHooks(tableName, typeName, options) {
  const lines = [];
...
function generateHooks(schemas, options = {}) {
  const {
  ...
  const typeImports = [];
  for (const [schemaName, schema] of schemas) {
    const namespace = pascalCase2(schemaName);
    for (const tableName of Object.keys(schema.tables)) {
      const typeName = pascalCase2(tableName);
  ...
  if (typeImports.length > 0) {
    ...
    for (const [schemaName] of schemas) {
      namespaces.add(pascalCase2(schemaName));
    }
  ...
  for (const [schemaName, schema] of schemas) {
    const namespace = pascalCase2(schemaName);
    for (const tableName of Object.keys(schema.tables)) {
      const typeName = `${namespace}.${pascalCase2(tableName)}`;
...
async function generateTypesFromRegistry(config, options) {
  ...
  try {
    ...
    if (options.includeHooks) {
      ...
      await writeFile2(hooksOutputPath, hooks, "utf-8");
  ...
  const regenerateTypes = async (registry2, reason) => {
    try {
      ...
      if (options.includeHooks) {
        ...
        await writeFile2(hooksOutputPath, hooks, "utf-8");
        console.log(
          `[${(/* @__PURE__ */ new Date()).toLocaleTimeString()}] ${reason} - Regenerated React Query hooks`
        );
      }
    } catch (error) {
      console.error(
...
function parseIntOrUndefined(value) {
  return value ? Number.parseInt(value, 10) : void 0;
}
async function handleMigrateUp(config, args) {
...
async function handleMigrateDown(config, args) {
  await runMigrations(config, {
    direction: "down",
    scope: args.scope,
    templateKey: args["template-key"],
...
async function handleMigrateStatus(config, args) {
  await getMigrationStatus(config, {
    scope: args.scope,
    templateKey: args["template-key"]
  });
}
async function handleMigrateVerify(config, args) {
  await verifyMigrations(config, {
    scope: args.scope,
    templateKey: args["template-key"]
  });
}
async function handleMigrateCreate(config, args) {
  if (!args.name) {
...
async function handleModuleList(config, _args) {
  await listModules(config);
}
async function handleModuleRegister(config, args) {
...
async function handleMigrateModules(config, args) {
